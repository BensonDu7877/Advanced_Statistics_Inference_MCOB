...
Answer: 
```{r}
### When the number of Mileage increases by 1, the price of an used car will decrease by 1.670e-02.
```
...
Answer: 
```{r}
### The eliminated predictors include Metallic, CC, Automatic, and Doors.
```
...
Answer:
```{r}
### The MAE for lm_full and lm_bwd, respectively, are 1058.757 and 1069.966.
```
...
Answer:
```{r}
### Yes, the log-transformation does help improve the prediction performance. Not only it has a lower
### MAE, but also there is a lower MAPE for the log-transformation, which means there will be less
### mean absolute percentage for the model to generate error.
```
...
total_data <- read.csv("./Carseats-2.csv", header=T, stringsAsFactors=T)
```
...
Answer:
```{r}
### The MAE for lm_full and lm_bwd are 0.8585053 and 0.8597975, respectively.
```
...
Answer:
```{r}
### I prefer lm_bwd, for it has the lower MAE and MAPE, which means this kind of model will have less
### chance to make mistakes while predicting.
```
