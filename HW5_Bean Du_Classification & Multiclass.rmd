...
rm(list=ls())
total_data <- read.csv("~/Desktop/STAT2022/dataset/Default.csv", header=T, stringsAsFactors=T)
```
...
summary(total_data)
### There are 333 customers default on their credit card debt.
```
...
### According to the result of BIC, the 'income' is removed.
```
...
library(gam)
gam1 <- gam(default~s(balance,df=4)+s(income,df=4)+student, family='binomial', data=train_data)
plot(gam1, col='blue')
```
...
```{r}
lm_full_acc[4]$byClass["Sensitivity"]
gam1_acc[4]$byClass["Sensitivity"]
### The sensitivities of both models go up after changing the threshold value. The prices we need to pay are   ### both  0.7723577.
```
...
library(nnet)
lm1 <- multinom(Species~Sepal.Length+Sepal.Width+Petal.Length+Petal.Width, data=train_data)
```
...
x_test_nn <- cbind.data.frame(test_data$Species, x_test_nn)
```
...
library(caret)
class_labels <- levels(test_data$Species)
...
Answer: 
```{r}
### I think the lm1 model may be a better choice because it is easier to understand than the nn1 model.
```
