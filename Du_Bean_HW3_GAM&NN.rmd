...
total_data <- read.csv("~/Desktop/STAT2022/dataset/Credit.csv")
```
...
lm_null <- lm(Balance~., data = train_data)
```
...
gam_null <- gam(Balance~s(Income,df=4)+s(Limit,df=4)+s(Rating,df=4)+s(Cards,df=4)+
              s(Age,df=4)+s(Education,df=4)+Student,data=train_data)
```
...
colnames(x_train_nn)[1] <- 'Sd_Balance'
```
...
library(neuralnet)
set.seed(7)
nn_null <- neuralnet(Sd_Balance~Income+Limit+Rating+Cards+Age+Education+StudentYes, data=x_train_nn, hidden=c(4,2)) 
plot(nn_null)
```
...
library(forecast)
lm_pred  <- predict(lm_null, newdata = test_data)
gam_pred <- predict(gam_null, newdata = test_data)
nn_pred  <- predict(nn_null, newdata = x_test_nn)[,1]*y_max
```
...
Answer:
```{r}
### The MAE of lm_null, gam_null, and nn_null are 84.22484, 51.80754, and 29.34589, respectively.
```
...
Answer: 
```{r}
### I prefer the nn_null model for it has the lowest MAE and RMSE , which means there will be lower 
### possibility to make mistakes while using this model.
```
